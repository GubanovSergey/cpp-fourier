...
int main() {    
    ...
    FFT<complex<int>> transformer(coeffs.begin(), coeffs.end());//by copy
    
    auto result = transformer.transform();
    std::cout << "fourier coeffs:\n" << result << std::endl;   
    
    FFT<complex<int>> transformer2(std::make_move_iterator(result.begin()), std::make_move_iterator(result.end())); //by move
